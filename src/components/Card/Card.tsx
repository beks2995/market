        ...
        <div>
            <div className="card__top">
                <div onClick={(e: MouseEvent<HTMLDivElement>) => clickHandle && clickHandle(el)}>
                    {
                        inFavorited && inFavorited.find((item: Idata) => item.name === el.name)?.isFavorited
                            ?
            ...
            <div className="card__estimation">
                ...
                {isAdminPage && el.id !== undefined && (
                    <button onClick={() => deleteHandle(el.id as string)} className="delete-button">
                        Удалить
                    </button>
                )}
            </div>
